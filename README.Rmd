...
```{r diving_fits_regularized,dependson=c('diving_fits'),eval=TRUE,echo=TRUE}
library(broom)
library(tidyr)
library(knitr)
# no regularization
...
```{r diving_fits_regularized_two,dependson=c('diving_fits_regularized'),eval=TRUE,echo=TRUE}
# no regularization
mod0 <- hensm(Finish ~ cut_age + country + home_advantage,data=fitdat,weights=weight,group=EventId,ngamma=3)
# l1 regularization
mod0_l1 <- hensm(Finish ~ cut_age + country + home_advantage,data=fitdat,weights=weight,group=EventId,ngamma=3,
