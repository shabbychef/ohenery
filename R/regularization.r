...
.check_regularization <- function(coef, reg_wt, reg_zero, reg_power, reg_coef_idx) {
	if (is.null(reg_wt)) {
		return
	}
...
.regularization_term <- function(coef, reg_wt, reg_zero, reg_power, reg_coef_idx) {
	if (is.null(reg_wt)) {
		return(0)
	}
...
.regularization_grad <- function(coef, reg_wt, reg_zero, reg_power, reg_coef_idx) {
	if (is.null(reg_wt)) {
		return(0)
	}
	gradbits <- - reg_wt * reg_power * (abs(coef[reg_coef_idx] - reg_zero)^(reg_power-1)) * sign(coef[reg_coef_idx] - reg_zero)
